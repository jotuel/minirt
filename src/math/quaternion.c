...
t_quat quat_normalize(t_quat q) {
  ...
  return (t_quat){.w = q.w / norm,
                  .v = (t_vec3){q.v.x / norm, q.v.y / norm, q.v.z / norm}};
}
...
t_vec3 quat_rot(t_quat q, t_vec3 v) {
  ...
  return ((t_vec3){.x = ww * v.x + 2 * wy * v.z - 2 * wz * v.y + xx * v.x +
                        2 * wx * v.y + 2 * xy * v.z,
                   .y = 2 * wz * v.x - ww * v.y + 2 * wy * v.z + yy * v.y +
                        2 * wy * v.x + 2 * yz * v.z,
                   .z = 2 * wy * v.x - 2 * wz * v.y + ww * v.z + zz * v.z +
                        2 * xz * v.x + 2 * yz * v.y});
}
