...
t_color diffuse_color(t_intersection hit, t_map *map, t_vec3 l_dir, t_vec3 normal)
{
	...
	light_dot_normal = vec3_dot(l_dir, normal);
...
t_color specular_color(t_ray r, t_map *map, t_vec3 l_dir, t_vec3 normal)
{
	...
	re_dot_eye = vec3_dot(re, r.dir);
	if (re_dot_eye <= 0)
		specular = (t_color){0};
	else
	{
		factor = powf(re_dot_eye, 50.0);
		specular = color_scale(color_scale(color_scale(specular, map->light->intensity), 0.8), factor);
	}	
	return (specular);
}