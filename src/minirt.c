...
void cast_rays(void *ptr)
{
	...
	while (w < map->img->width)
	{
		...
		{
			p_cen = vec3_add(
			    vec3_add(map->camera->pixel00_pos, vec3_scale(map->camera->pixel_delta_u, h)),
			    vec3_scale(map->camera->pixel_delta_v, w));
			ray_direction = vec3_unit(vec3_subtract(p_cen, map->camera->lookfrom));
			ray = (t_ray) {.origin = map->camera->lookfrom, .dir = ray_direction};
